...
(defn create-db-researcher
  "Returns test db entity"
  [conn test]
  (let [researcher-tempid (d/tempid :test)]
    (-> @(d/transact conn [{:db/id researcher-tempid
                            :agent/type :agent.type/researcher
                            :test/_agents (simu/e test)}])
        (simu/tx-ent researcher-tempid))))

(defn create-db-job
  "Returns test db entity"
  [conn job-spec researcher-spec researcher-id]
  @(d/transact conn [{:db/id (d/tempid :test)
                      :agent/_actions researcher-id
                      :action/atTime (:at-ms job-spec)
                      :action/type :action.type/job
                      :job/username (:username researcher-spec)
...
(defmethod sim/create-test :model.type/cook
  ...
  (let [test (create-db-test conn {:db/id (d/tempid :test)
    ...
    (doseq [r (:researchers schedule)]
      (let [researcher-id (create-db-researcher conn test)]
        (doseq [j (:jobs r)] (create-db-job conn j r (simu/e researcher-id)))))
...
(defn researchers-for-profile
  [test-duration profile]
  (into [] (map (partial random-researcher test-duration profile)
                (:usernames profile))))

(defn random-schedule
  [{:keys [label duration-seconds researcher-profiles] :as settings}]
  {:label label
   :duration-seconds duration-seconds
   :researchers (->> researcher-profiles
                     (map (partial researchers-for-profile duration-seconds))
                     (apply concat)
                     (into []))})
...
(defn- format-schedule
  ...
  [db [id label duration num-researchers]]
  {"ID" id
   "Label" label
