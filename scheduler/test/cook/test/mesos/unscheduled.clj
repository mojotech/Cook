...
(ns cook.test.mesos.unscheduled
  ...
  (:require [cook.mesos.unscheduled :as u]
            ...
            [cook.test.mesos.schema :refer (restore-fresh-database!
                                            create-dummy-job
...
(deftest test-exhausted-retries
  (let [job {:job/max-retries 3}]
    (is (nil? (u/check-exhausted-retries job))))
  (let [job {:job/max-retries 3
             :job/instance (repeat 5 {:instance/status :instance.status/failed})}
        result (u/check-exhausted-retries job)]
    (is (string? (first result)))
    (is (= (last result)
           {:max-retries 3 :instance-count 5}))))
  ...
  (let [conn (restore-fresh-database! datomic-uri)
    ...
    (is (= (u/check-fenzo-placement conn
           ...
           ["The job couldn't be placed on any available hosts."
            {:reasons
             [{:reason "Not enough Memory available." :host_count 14}
              {:reason "Not enough CPU available." :host_count 8}
